...
class NewCanvasDialog::Private
{
        ...
        orientationGroup(nullptr)
    {
        ...
        if (sl.count() == 0)
        {
            qWarning() << "No template directory found!";
            return;
        }
        ...
        {
            QString tmp = dir + QLatin1String("/") + pair->second;
...
void NewCanvasDialog::paperSizeSelected(QListWidgetItem* current, QListWidgetItem* /*previous*/)
{
    ...
    else
    {
        ...
        TemplatesModel* const model  = new TemplatesModel();
        ...
        {
            this->loadTemplatesList(QLatin1String("/") + paper.second + QLatin1String("/v"), model);
        }

        if (!d->verticalButton->isChecked())
        {
            this->loadTemplatesList(QLatin1String("/") + paper.second + QLatin1String("/h"), model);
        }
    }
}
...
void NewCanvasDialog::setupUI()
{
    ...
    connect(d->paperSize, SIGNAL(currentItemChanged(QListWidgetItem*,QListWidgetItem*)),
            this, SLOT(paperSizeSelected(QListWidgetItem*,QListWidgetItem*)));

    foreach (QListWidgetItem* const i, d->paperSizes)
    {
    ...
    QVBoxLayout* const rightLayout = new QVBoxLayout();
    ...
    connect(d->horizontalButton, SIGNAL(toggled(bool)),
            d->canvasSize, SLOT(setHorizontal(bool)));

    connect(d->verticalButton, SIGNAL(toggled(bool)),
            d->canvasSize, SLOT(setVertical(bool)));
...
void NewCanvasDialog::loadTemplatesList(const QString& path, TemplatesModel* const model)
{
    ...
    qDebug() << "Template directories:" << sl;

    QDir diro(sl.first() + path);
